...
class Trainer:
	...
	def __init__(self,signalpath,backgroundpath,perm, channel, traintest, BDTOptions,variables,unusedvariables=[],verbose=False,weightfile="weights/weights.xml"):
		...
		self.SignalWeightExpression="xs_weight*PU_weight*b_weight"
		self.BackgroundWeightExpression="xs_weight*PU_weight*b_weight"
		#self.SignalWeightExpression="PU_weight*b_weight"
		#self.BackgroundWeightExpression="PU_weight*b_weight"
...
#              factory.AddVariable(str(var))
		...
		treeS=ROOT.TChain("odd/" + self.perm + "/" + self.channel + "/MVAtree")
		treeS.Add(self.signalpath)
		treeB=ROOT.TChain("odd/" + self.perm + "/" + self.channel + "/MVAtree")
		...
		if self.totest:
		  ...
		  if (self.traintest == "Test"):
			  treeSTest=ROOT.TChain("even/" + self.perm + "/" + self.channel + "/MVAtree")
			  treeSTest.Add(self.signalpath)
			  treeBTest=ROOT.TChain("even/" + self.perm + "/" + self.channel + "/MVAtree")
			  treeBTest.Add(self.backgroundpath)
		  if (self.traintest == "Train"):
			  treeSTest=ROOT.TChain("odd/" + self.perm + "/" + self.channel + "/MVAtree")
			  treeSTest.Add(self.signalpath)
			  treeBTest=ROOT.TChain("odd/" + self.perm + "/" + self.channel + "/MVAtree")
			  treeBTest.Add(self.backgroundpath)

		treeS.SetBranchStatus("*",0)
		#treeS.SetBranchStatus("Weight",1)
		#treeS.SetBranchStatus("Weight_PV",1)
		treeS.SetBranchStatus("b_weight",1)
		treeS.SetBranchStatus("PU_weight",1)
		treeS.SetBranchStatus("xs_weight",1)
		...
		for var in variables+["xs_weight","b_weight","PU_weight"]:
		  if ":=" in var:
			  continue
		...
		treeB.SetBranchStatus("PU_weight",1)
		treeB.SetBranchStatus("xs_weight",1)
		...
		treeB.SetBranchStatus("b_weight",1)
		...
		if self.totest:
		  ...
		  treeSTest.SetBranchStatus("PU_weight",1)
		  treeSTest.SetBranchStatus("xs_weight",1)
		  ...
		  treeSTest.SetBranchStatus("b_weight",1)
		  ...
		  treeBTest.SetBranchStatus("PU_weight",1)
		  treeBTest.SetBranchStatus("xs_weight",1)
		  ...
		  treeBTest.SetBranchStatus("b_weight",1)
